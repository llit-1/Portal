...
namespace Portal.Controllers
{
    public class TimesheetsFactoryController : Controller
    {
        ...
        public IActionResult TimesheetsMain(string loc = "", int month = 0, int year = 0)
        {
            ...
            try
            {
                // Если дата не указана, то ставим текущую
                if (month == 0 || year == 0)
                {
                ...
                // Проверяем привязанные к пользователю точки (заводы и цеха)
                ...
                // Если точка не передана, то берем первую подходящую и по ней собираем информации далее
                if (loc == "")
                {
                ...
                // Все сотрудники с указанной точки
                ...
                // Начинаем заполнять данные по каждому сотруднику
                ...
                {
                    // Сюда попадают все записи по одному сотруднику, даже если он отработал на разных должностях в этот период
                    // Создаются группы по должностям
        ...
        public IActionResult TimesheetsSave(string json)
        {
            ...
            try
            {
                ...
                foreach (var item in jsonData.Data)
                {
                    ...
                    // Удаляем все записи за выбранный месяц у выбранного человека
                    ...
                    for (var i = 0; i < item.Value.Entries.Count; i++)
                    {
                        ...
                        // Выбор половины дня (1 = день, 0 = ночь)
                        if ((i + 1) % 2 != 0)
                        {
