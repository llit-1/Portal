...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.init = function () {
    ...
    this._elemWrapper = $('#' + this.comboTreeId + 'Wrapper');
      ...
      this._elemArrowBtn = $('<div id="' + this.comboTreeId + 'ArrowBtn" class="comboTreeArrowBtn" type="button"><span class="mdi mdi-chevron-down comboTreeArrowBtnImg" style="font-size:20px"></span></div>');
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (!isSelectable || isThereSubs)
        selectableClass = 'not-selectable';

    itemHtml += '<LI id="' + this.comboTreeId + 'Li' + sourceItem.id + '" class="ComboTreeItem' + (isThereSubs?'Parent':'Chlid') + '"> ';

    if (isThereSubs)
        itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '<span class="mdi mdi-folder"></span>' : '<span class="mdi mdi-folder-open"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';
    else
        itemHtml += '<span class="comboTreeParentPlus"><span class="mdi mdi-file-outline"></span></span>';
  ...
  ComboTree.prototype.bindings = function () {
    ...
    this._elemItemsTitle.on('click', function(e){
        ...
        // вместо выбора каталога, расхлопываем его
        var parent = $(this).parent();        
        if (parent.attr('class') == "ComboTreeItemParent") {
            _this.toggleSelectionTree(parent);
        }
        else {
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined){
      if ($(subMenu).is(':visible'))
          $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-folder"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
          $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-folder-open"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
